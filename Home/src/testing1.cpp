...
void updateGridViewport(Grid &grid, int rows, int cols) {
    // Calculate viewport based on drawGrid() settings
    int left = grid.getOffsetX();
    int top = grid.getOffsetY();
    int right = left + cols * CELL_SIZE;
    int bottom = top + rows * CELL_SIZE;

    // Set viewport for the grid area
    setviewport(left, top, right, bottom, 1);

    // Clear and redraw the grid area
    clearviewport();
    grid.drawGrid();
        ...
        while (true) {
            ...
            if (ismouseclick(WM_LBUTTONDOWN)) {
                ...
                if (x >= leftX && x <= leftX + BUTTON_WIDTH && y >= rowsPlusY && y <= rowsPlusY + BUTTON_HEIGHT && rows < 15) {
                    rows++;
                    redrawMenu = true;
                } else if (x >= leftX && x <= leftX + BUTTON_WIDTH && y >= rowsMinusY && y <= rowsMinusY + BUTTON_HEIGHT && rows > 8) {
                    rows--;
                    redrawMenu = true;
                } else if (x >= rightX && x <= rightX + BUTTON_WIDTH && y >= colsPlusY && y <= colsPlusY + BUTTON_HEIGHT && cols < 25) {
                    cols++;
                    redrawMenu = true;
                } else if (x >= rightX && x <= rightX + BUTTON_WIDTH && y >= colsMinusY && y <= colsMinusY + BUTTON_HEIGHT && cols > 8) {
                    cols--;
                    redrawMenu = true;
        ...
        if (winSeq.hasWinner) {
            ...
            if (winSeq.hasWinner) {
                ...
                displayDrawMessage();
                        ...
                        countdown = timerMode; // Reset timer for the next player
